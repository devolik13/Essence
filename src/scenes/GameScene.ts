...
export class GameScene extends Phaser.Scene {
  ...
  /** Открыть редактор карт сразу при старте сцены (кнопка «Lab Editor»). */
  private editorOnStart = false;
  private labWave = 0;
  ...
  init(data?: { zoneId?: string; spawnX?: number; spawnY?: number; isNewGame?: boolean; starterBodyId?: string; starterWeapon1?: string; starterWeapon2?: string; characterName?: string; editorOnStart?: boolean }) {
    this.editorOnStart = data?.editorOnStart ?? false;
  ...
  create() {
    ...
    if (this.currentZone.id === 'lab' && !this.editorOnStart) this.initLabDungeon();
    ...
    if (this.editorOnStart) {
      this.time.delayedCall(300, () => this.mapEditor?.toggle());
    }
  ...
  private spawnCreatures() {
    ...
    // Питч-демо: учителя демо-маршрута прямо у деревни, чтобы не бегать
    // через всю карту. В обычной игре их здесь нет.
    // — Игнис слева (босс + тело-проводник в лабораторию)
    // — Искра у южного выхода (учитель заклинания Искра)
    if (this.pitchMode && this.currentZone.id === 'village') {
      const ignisDef = CREATURE_DB['ignis'];
      if (ignisDef) this.creatures.push(new Creature(this, 2350, 2800, ignisDef));
      const sparkDef = CREATURE_DB['spark'];
      if (sparkDef) this.creatures.push(new Creature(this, 3200, 3320, sparkDef));
    }
